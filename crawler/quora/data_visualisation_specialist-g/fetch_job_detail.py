...
try:
    ...
    channel.queue_declare(queue='quoraG_fetch_job_details', durable=True)   
    channel.queue_declare(queue='mongoG_jobs_save', durable=True)
    ...
    def callback(ch, method, properties, body):
        ...
        try:
            ...
            channel.basic_publish(exchange='', routing_key='mongoG_jobs_save', body=json.dumps(job))
        except Exception as e:
            print("Quora...Unable to parse html::", e)

    # channel.basic_consume(callback, 'remote_fetch_job_details',  no_ack=True)
    channel.basic_consume(
       queue='quoraG_fetch_job_details', on_message_callback=callback, auto_ack=False)
