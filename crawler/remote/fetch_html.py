...
def fetch_data():
    try: 
        urls = json.loads(os.getenv("REMOTE_SEARCH_URL"))
        print(len(urls))
        ...
        for url in urls:
            ...
            channel.basic_publish(exchange='', routing_key='remote_html_parse', body=json.dumps({'html':body.decode("utf-8"), 'domain': domain}))
            print("Remote data sent to parse queue...")
        ...
        error = {
            "status": "Remote........... Error occured while fetching html",
            "errorMsg": e
        }
        print("Error: ",error)
