...
try:
    ...
    def callback(ch, method, properties, body):
        ...
        jobs = soup.find_all("a", class_="job-list d-none d-md-flex") if soup.find("a", class_="job-list d-none d-md-flex") else ''
        ...
        for item in jobs:
            ...
            if job_link is None:
                continue
            job['job_link'] = 'https://nocodery.com' + job_link
            print("\njob::", job)

            channel.basic_publish(exchange='', routing_key='no_codery_fetch_job_details', body=json.dumps(job))
        else:
            print("no Jobs data !!")

    channel.basic_consume(
        queue='no_codery_html_parse', on_message_callback=callback, auto_ack=True)
    ...
    error = {
        "status": "No Codery......... Error occured while parsing html",
        "errorMsg": e
    }
    print("Error: ",e)
