...
try:
    ...
    channel.queue_declare(queue='zip_recruiter_fetch_job_details', durable=True)
    ...
    def callback(ch, method, properties, body):
        ...
        for item in res:
            ...
            channel.basic_publish(exchange='', routing_key='zip_recruiter_fetch_job_details', body=json.dumps(job))

    # channel.basic_consume(callback, 'zip_recruiter_html_parse',  no_ack=False)
    channel.basic_consume(
       queue='zip_recruiter_html_parse', on_message_callback=callback, auto_ack=True)
